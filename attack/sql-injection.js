...
sqlInjection
  .targetFormInput = async function (script, url, inputs, idx = inputs.length - 1, result = []) {
    ...
    let inputsIndex = idx;
    // Recursively fills input with all the sql scripts
    async function fillInput(s = sqlScript.length - 1) {
      let index = s;
      if (index < 0) return;
      ...
      await horseman
        ...
        // **** End ****
        .type(`input[name=${inputs[inputsIndex]}]`, sqlScript[index])
